...
// 初始化
let musicManager;
let musicManagerInitialized = false;

// 初始化音乐管理器（可被外部调用）
function initMusicManager(options = {}) {
    ...
    if (!adminToken) {
        console.debug(`[${timestamp}] 音乐管理页面初始化检查失败:`, {
            ...
            musicPanelExists: !!musicPanel,
            musicListContainerExists: !!musicListContainer,
        ...
        if (retryCount < maxRetries) {
            ...
            setTimeout(() => {
                initMusicManager({ retryCount: retryCount + 1, maxRetries });
            }, 200);
        } else {
            console.warn(`[${timestamp}] 音乐管理页面元素未找到，已重试 ${maxRetries} 次，跳过初始化`);
        }
        return;
    }
    ...
    try {
        ...
        console.error(`[${timestamp}] 音乐管理器初始化失败:`, error, {
            url: currentUrl,
